...
version 1.11.0.RELEASEで利用するOSSの一覧を以下に示す。

.. note::

  version 1.6.1.RELEASEより、\ `Spring Boot <https://spring.io/projects/spring-boot>`_\ が提供する\ `spring-boot-dependencies <https://docs.spring.io/spring-boot/3.4.1/reference/using/build-systems.html#using.build-systems.dependency-management>`_\ の\ ``<dependencyManagement>``\ をインポートする構成を採用している。
  ...
  * Spring Frameworkと相性のよいOSSライブラリ
  ...
  なお、version 1.11.0.RELEASEでは\ `Spring Boot 3.4.1 <https://docs.spring.io/spring-boot/3.4.1>`_\ に依存しており、管理されるライブラリは\ `Spring Boot Reference Guide - Appendix F. Dependency versions <https://docs.spring.io/spring-boot/3.4.1/appendix/dependency-versions/coordinates.html>`_\ の通りとなる。
...
| Macchinetta Server Framework (1.x)では、\ `TERASOLUNA Server Framework for Java (5.x) <https://github.com/terasolunaorg>`_\ が提供する\ `共通ライブラリ <https://github.com/terasolunaorg/terasoluna-gfw/tree/5.10.0.RELEASE>`_\ を使用する。（以降「共通ライブラリ」と記載する。）
...
.. note::
  ...
  version 1.11.0.RELEASEでは TERASOLUNA Server Framework for Java 5.10.0.RELEASE の共通ライブラリを使用している。
