...
| SOAPの仕様についての詳細は、\ :url_w3c:`W3C -SOAP Specifications- </soap/>`\ を参照されたい。
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
| \ :url_jakarta_xml_ws:`Jakarta XML Web Services <>`\ とは、SOAPなどを使ったWebサービスを扱うためのJava標準APIである。
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
| \ |framework_name|\ 1.8 までは、Spring Frameworkが提供するJakarta XML Web Servicesの連携機能を利用した実装を推奨していたが、Spring Framework 6から、\ ``org.springframework.remoting``\ パッケージが削除され、Jakarta XML Web Servicesの連携機能が利用できなくなった。（\ :url_spring_framework_issues:`Spring#27422 </27422>`\） 
| それに伴い、\ |framework_name|\ 1.11 からはJakarta XML Web Servicesでの実装を推奨している。
| 
...
| WSDLの詳細は、\ :url_w3c:`W3C -Web Services Description Language (WSDL)- </2001/NOTE-wsdl-20010315>`\ を参照されたい。
...
| そのため、本節で説明するSOAPサーバは、Jakarta XML Web Servicesの実装プロダクトとして\ :url_apache_cxf:`Apache CXF <>`\ を使用している。
...
| 入力チェックには\ :ref:`メソッドバリデーション<ValidationMethodValidation>`\ を使用するため、以下の定義を追加する。
...
| 仕様ではないが、Namespaceとパッケージの命名について、\ :url_xml_namespace_mapping:`XML Namespace Mapping(Red Hat JBoss Fuse) <>`\ にまとまっている。
...
.. list-table::
    ...
    * - | (1)
      - | \ ``org.apache.cxf.transport.servlet.CXFServlet``\ 内で動作するWebサービスのエンドポイントを定義する。
        ...
        | 属性の詳細については\ :url_cxf_jaxws:`Apache CXF JAX-WS Configuration </>`\ を参照されたい。
...
| メソッドバリデーションの詳細については\ :ref:`ValidationMethodValidationOnSpringFrameworkHowToUseApplyTarget`\ を参照されたい。
...
.. list-table::
    ...
    * - | (2)
      - | ここで、\ ``WebFaultException``\ の親クラスを\ ``RuntimeException``\ ではなく\ ``Exception``\ としているのは、\ :url_jakarta_xml_ws:`Jakarta XML Web Services 4.0 -3.7. Service Specific Exception <#j2w11except>`\ において、
...
| 詳細な定義は \ :url_w3c:`W3C -SOAP Message Transmission Optimization Mechanism- </soap12-mtom/>`\ を参照されたい。
| 以下にその方法を記述する。
    ...
    * - | (1)
      - | 取得した\ ``RequestContext``\ に対して、タイムアウトの設定を追加する。
        ...
        .. note:: **タイムアウト定義に使用するキーについて**
            ...
            詳細は\ :url_jaxws_timeoutkey:`JAX_WS-1166 Standardize timeout settings <>`\ を参照されたい。
...
| 削除対象は、「:doc:`../../ImplementationAtEachLayer/BlankProject` の :ref:`BlankProjectMultiProjectConfiguration`」を参照されたい。
...
.. note::
    ...
    Apache CXFを利用する場合は、org.glassfish.jaxb:jaxb-runtimeのJAXBが自動的に依存関係に追加されることを確認しているが、本ガイドラインでは\ :ref:`remove-jaxb-from-java11`\ で説明のとおり、com.sun.xml.bind:jaxb-implを使用することを前提とするため、\ :url_terasoluna_dependencies:`terasoluna-gfw-dependencies <>`\ で定義しているApache CXFからJAXBの依存関係を抜いている。
...
.. list-table::
    ...
    * - | (3)
      - | WebServiceで定義する\ ``portName``\ は、WSDLの\ ``<wsdl:port>``\ 要素に対応する。
        ...
        | 詳しくは、「\ :url_w3c:`Web Services Description Language (WSDL) 1.1 </2001/NOTE-wsdl-20010315>`\ 」を参照されたい。
    * - | (4)
    ...
    * - | (4)
      - | 取得した\ ``RequestContext``\ に対して、エンドポイントアドレスの設定を追加する。
        ...
        .. note::
          ...
          \ ``BindingProvider``\ の詳細については \ :url_jakarta_xml_ws:`Jakarta XML Web Services 4.0 -4.2 jakarta.xml.ws.BindingProvider- <#bindingproviderif>`\ を参照されたい。
