...
ここでは、バッチ処理によってロックされているデータに対して、オンライン処理で更新する例を示す。
  ...
  .. figure:: ./images_ExclusionControl/ExclusionControl-problem4.png
...
* 買いたい商品を5個ずつ同時に購入したが、少しUserAの方が早く購入ボタンを押下したため、User Aが先に購入し、User Bが次に購入する。
  ...
  .. figure:: ./images_ExclusionControl/update-for-db-line-lock.png
...
| アプリケーションでは、更新件数が0件の場合、購入処理をロールバックし、User Bに再度実行を促す。
  ...
  .. figure:: ./images_ExclusionControl/update-for-db-line-lock-not-enough.png
...
.. note:: \ **Versionカラムとは**\
  ...
  Versionカラムは、数値以外に最終更新タイムスタンプで代用することもできる。しかし、タイムスタンプを用いると、同時に処理が実行された際の、一意性が保証されない。そこで、確実な一意性を求める場合、Versionカラムは、数値を使用する必要がある。
...
.. warning::
  ...
  楽観ロックを行う場合、IDとVersion以外の条件を加えて更新・削除するのは適切でない。なぜなら更新できなかった場合に、Versionが一致しないことが理由なのか、別の条件に一致しないのが理由なのか、判断できないためである。
...
* それぞれ表示された在庫数に対して、仕入れた数を加算した値を更新フォームに入力し、更新を行う。
  ...
  .. figure:: ./images_ExclusionControl/Optimistic-lock-flow.png
...
* バッチ処理は5秒後(タイムアウト前)に終了する。
  ...
  .. figure:: ./images_ExclusionControl/Pessimistic-lock.png
...
| バッチ処理の終了まで待たずに排他エラーとなる。
  ...
  .. figure:: ./images_ExclusionControl/Pessimistic-lock-timeout.png
    :alt: Pessimistic lock
    :width: 90%
    :align: center
...
| 悲観ロックの解放を待つことなく、すぐに排他エラーとなる。
  ...
  .. figure:: ./images_ExclusionControl/Pessimistic-lock-nowait.png
    :alt: Pessimistic lock
    :width: 90%
    :align: center
...
以下(1)～(5)の流れで、複数のトランザクションから、同一テーブルのレコードに対してロックを行うと、デッドロックとなる。
  ...
  .. figure:: ./images_ExclusionControl/Dead-Lock-Record.png
...
| 基本的な考え方は、 :ref:`Dead-Lock-Record` と同じである。
  ...
  .. figure:: ./images_ExclusionControl/Dead-Lock-Table.png
    :alt: Dead Lock Table
    :width: 90%
    :align: center
...
楽観ロックの失敗時には、\ ``org.springframework.dao.OptimisticLockingFailureException``\ が発生するため、Controllerで適切にハンドリングする必要がある。
