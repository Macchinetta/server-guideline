...
.. note:: \ **プロジェクト構成の原則**\
  ...
  詳細についてはチュートリアル(\ :url_tutorial:`JavaConfig </JavaConfig/secure-login-demo>`\ /\ :url_tutorial:`XMLConfig </XMLConfig/secure-login-demo>`\ )を参考にされたい。
...
4. Tomcatのリソース機能を使用して、 /etc/foo/bar/\*.jar をクラスパスに追加する。
  ...
  * | [CATALINA_HOME]/conf/Catalina/localhost/[contextPath].xmlファイルに下記の定義を追加する。
    | 詳しくは、\ :url_tomcat:`The Resources Component </config/resources.html>`\ と\ :url_atrs:`atrs-envのconfigsフォルダ </atrs-env/configs>`\ を参考されたい。
  * リソースの設定例：
...
.. note::
  ...
  pom.xmlファイルの<version>タグの変更は\ :url_mojohaus:`versions-maven-plugin </versions-maven-plugin/>`\ で可能である。
