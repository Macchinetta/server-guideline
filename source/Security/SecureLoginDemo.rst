...
* \ |framework_name|\を利用して代表的なセキュリティ要件を満たすための実装方法の例
...
| ここでは分類ごとで要件の実現のために必要最小限なコード片のみを掲載している。コード全体を確認したい場合は\ :url_tutorial:`GitHub(JavaConfig) </JavaConfig/secure-login-demo>`\ /\ :url_tutorial:`GitHub(XMLConfig) </XMLConfig/secure-login-demo>`\ を参照すること。
| 本アプリケーションを動作させるための初期データ登録用SQLはここ(\ :url_tutorial:`JavaConfig-JSP </JavaConfig/secure-login-demo/secure-login-jsp/secure-login-jsp-env/src/main/resources/database>`\ /\ :url_tutorial:`JavaConfig-Thymeleaf </JavaConfig/secure-login-demo/secure-login-thymeleaf/secure-login-thymeleaf-env/src/main/resources/database>`\ /\ :url_tutorial:`XMLConfig-JSP </XMLConfig/secure-login-demo/secure-login-xmlconfig-jsp/secure-login-xmlconfig-jsp-env/src/main/resources/database>`\ /\ :url_tutorial:`XMLConfig-Thymeleaf </XMLConfig/secure-login-demo/secure-login-xmlconfig-thymeleaf/secure-login-xmlconfig-thymeleaf-env/src/main/resources/database>`\ )に配置されている。
...
.. tip::
  ...
  * \ :url_soumu_password:`安全なパスワードの設定・管理 </>`\
  * \ :url_nist_password:`NIST Special Publication 800-63B <>`\
...
* 初期パスワード、パスワード有効期限切れの判定
    ...
    public class AccountSharedServiceImpl implements AccountSharedService {
        ...
        public boolean isCurrentPasswordExpired(String username) { // (5)
            ...
            if (passwordHistories.get(0).getUseFrom().isBefore(
                    LocalDateTime.now(dateFactory.tick()).minusSeconds(passwordLifeTimeSeconds))) { // (8)
                return true;
            }
  ...
  .. tip::
    ...
    Cache Abstraction については\ :url_spring_reference:`Spring Framework Documentation -Cache Abstraction- </integration/cache.html>`\ を参照すること。
      ...
      public class SpringMvcConfig implements WebMvcConfigurer {
          ...
          private void addPasswordExpirationCheckInterceptor(InterceptorRegistry registry,
              ...
              registry.addInterceptor(interceptor).addPathPatterns("/**") // (1)
                      .excludePathPatterns("/password/**", "/reissue/**", "/resources/**", "/*/*.html"); // (2) (3)
          }
  ...
  .. tabs::
    .. group-tab:: JSP
      ...
      .. code-block:: jsp
        ...
        <div id="wrapper">
            <span id="expiredMessage"> <t:messagesPanel /> </span> <!-- (1) -->
...
| そこで、パスワード入力チェック用のライブラリとして\ :url_passay:`Passay </>`\ を利用し、必要なBean Validationのアノテーションを作成する。
...
* Passayの検証規則の作成
  ...
  .. code-block:: java
    ...
    public class EncodedPasswordHistoryRule extends HistoryRule { // (1)
        ...
        @Override
        protected boolean matches(final String rawPassword, final PasswordData.Reference reference) { // (3)
  ...
  .. list-table::
    ...
    * - | (1)
      - | 新しいパスワードの二回の入力が一致しているかをチェックするためのアノテーション。
        | 詳細は\ :ref:`ValidationTerasolunaGfwList`\ を参照すること。
    * - | (2)
    ...
    public class AccountSharedServiceImpl implements AccountSharedService {
        ...
        public boolean isLocked(String username) {
            ...
            if (failureEvents.get(lockingThreshold - 1).getAuthenticationTimestamp().isBefore(
                    LocalDateTime.now(dateFactory.tick()).minusSeconds(lockingDurationSeconds))) { // (5)
                return false;
            }
        ...
        public class SpringSecurityConfig {
            ...
            public AuthenticationProvider authProvider(UserDetailsService loggedInUserDetailsService,
                ...
                DaoAuthenticationProvider authProvider =
                        new DaoAuthenticationProvider(loggedInUserDetailsService); // (1)
            ...
            public class SpringSecurityConfig {
                ...
                public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
                    ...
                    http.authorizeHttpRequests(authz -> authz.requestMatchers(antMatcher("/login")).permitAll()
                            ...
                            .requestMatchers(antMatcher("/unlock/**")).hasRole("ADMIN") // (1)
            ...
            public String home(@AuthenticationPrincipal LoggedInUser userDetails, Model model) { // (1)
                ...
                if (lastLoginDate != null) {
                    model.addAttribute("lastLoginDate",
                            lastLoginDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))); // (3)
                }
        ...
        <body>
            <div id="wrapper">
                ...
                <c:if test="${!empty lastLoginDate}"> <!-- (1) -->
                    <p id="lastLogin">Last login date is ${f:h(lastLoginDate)}.</p> <!-- (2) -->
                </c:if>
      ...
      .. code-block:: html
        ...
        <body>
            <div id="wrapper">
                ...
                <!--/* (1)(2) */-->
                <p id="lastLogin" th:if="${lastLoginDate} !=null" th:text="|Last login date is ${lastLoginDate}.|"></p>
          ...
          public String createAndSendReissueInfo(String username) {
              ...
              if (accountSharedService.exists(username)) { // (6)
                  ...
                  LocalDateTime expiryDate =
                          LocalDateTime.now(dateFactory.tick()).plusSeconds(tokenLifeTimeSeconds); // (9)
        ...
        public String createAndSendReissueInfo(String username) {
            ...
            if (accountSharedService.exists(username)) {
                ...
                uriBuilder.scheme(protocol).host(host).port(port).path(contextPath)
                        ...
                        .queryParam("token", info.getToken()); // (2)
      ...
      public class PasswordReissueServiceImpl implements PasswordReissueService {
          ...
          public String createAndSendReissueInfo(String username) {
              ...
              if (accountSharedService.exists(username)) {
                  ...
                  LocalDateTime expiryDate =
                          LocalDateTime.now(dateFactory.tick()).plusSeconds(tokenLifeTimeSeconds); // (2)
    ...
    .. code-block:: java
      ...
      public class PasswordReissueServiceImpl implements PasswordReissueService {
          ...
          public PasswordReissueInfo findOne(String token) {
              ...
              if (LocalDateTime.now(dateFactory.tick()).isAfter(info.getExpiryDate())) { // (2)
                ...
                <form:form action="${f:h(pageContext.request.contextPath)}/reissue/resetpassword" method="POST" modelAttribute="passwordResetForm">
                    <table>
                        <tr>
                            ...
                            <td>${f:h(passwordResetForm.username)} <form:hidden path="username" value="${f:h(passwordResetForm.username)}" /></td> <!-- (1) -->
                            <td></td>
                        </tr>
    ...
    .. group-tab:: JSP
      ...
      .. list-table::
        ...
        * - 項番
          - 説明
        * - | (1)
          - | \ ``InvalidCharacterException``\ はクライアントの入力に起因して発生する例外であるため、HTTPステータスコードを\ ``400``\ (Bad Request)に設定する
    ...
    .. code-block:: java
      ...
      public class DomainRestrictedURLValidator
          ...
          public void initialize(DomainRestrictedURL constraintAnnotation) {
              allowedDomains = new HashSet<String>(Arrays.asList(constraintAnnotation.allowedDomains())); // (2)
          }
...
| Springが提供しているAOPの実装方法は複数あるが、本アプリケーションでは\ :url_gfw:`共通ライブラリ <>`\ で提供しているロギング関連の部品の実装と合わせることを重視し、\ ``org.aopalliance.intercept.MethodInterceptor``\ を実装する方式を採用する。
...
* 上記で定義したアドバイスを\ ``@Service``\ の付与されたクラスに対して適用するための設定を行う
  ...
  .. note::
    ...
    Springが提供するAOP機能に関しては、\ :url_spring_reference:`Spring Framework Documentation -Aspect Oriented Programming with Spring- </core/aop.html>`\ を参照すること。
      ...
      .. code-block:: java
        ...
        public class SpringSecurityConfig {
            ...
            public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
                ...
                http.addFilterAfter(userIdMDCPutFilter(), AnonymousAuthenticationFilter.class); // (1)
...
* アドバイスを\ ``@Service``\ の付与されたクラスに対して適用するための設定を行う
  ...
  .. tabs::
    .. group-tab:: Java Config
      ...
      .. code-block:: java
        ...
        @Import({SecureLoginInfraConfig.class, SecureLoginCodeListConfig.class})
        public class SecureLoginDomainConfig {
            ...
            public Advisor serviceCallLoggingInterceptorAdvisor() {
                ...
                pointcut.setExpression("@within(org.springframework.stereotype.Service)"); // (2)
  ...
  .. note::
    ...
    詳細は\ :url_spring_reference:`Spring Framework Documentation -Understanding AOP Proxies- </core/aop/proxying.html#aop-understanding-aop-proxies>`\ を参照すること。
