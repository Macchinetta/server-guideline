...
\ :url_lombok:`Lombok </>`\ は、Java言語におけるボイラープレートコードをソースコードから排除するために使用するライブラリである。
...
| 使用するIDEによってインストール方法は異なるため、STS以外のIDEを使用する場合は、\ :url_lombok:`こちらのページ </download>`\ を参考にされたい。
...
* \ :url_lombok:`Lombokのダウンロードページ </download>`\
...
| Lombokを初めて使用する場合は、まず、Lombokの「\ :url_lombok:`Demo Video </>`\ 」を参照するとよい。
...
* \ :url_lombok:`Lombok features </features/all>`\
...
.. list-table::
  ...
  * - 9.
    - \ :url_lombok:`@lombok.extern.slf4j.Slf4j </features/log>`\
...
.. tip::
  ...
  \ ``@Value``\ アノテーションについては、\ :url_lombok:`Lombokのリファレンス </features/Value>`\ を参照されたい。
