...
| ファイルのアップロードは、Servlet 3.0からサポートされたファイルアップロード機能と、Spring Webから提供されているクラスを利用して行う。
 ...
 .. note::
    ...
    version 1.7.2.SP1.RELEASEまでで問題の発生を確認した実績のあるアプリケーションサーバは以下の通りである。
...
- :file:`web.xml`
 ...
 .. warning:: **Spring Security使用時の注意点**
    ...
    この動作を回避する方法が\ `Spring Security Reference -Include CSRF token in action- <https://docs.spring.io/spring-security/site/docs/5.2.8.RELEASE/reference/htmlsingle/#servlet-csrf-considerations-multipart-url>`_\ の中で紹介されているが、セキュリティ上のリスクを含む回避方法になるため、本ガイドラインでは回避策の適用は推奨していない。
...
.. warning::
    ...
    なお、Macchinetta Server Framework version 1.7.2.SP1.RELEASEで管理されているバージョンを使用すれば、CVE-2014-0050およびCVE-2016-3092で報告されている脆弱性は発生しない。
